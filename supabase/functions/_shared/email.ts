...
//
// Cần cấu hình các secret sau trong Supabase Project Settings > Edge Functions:
//   RESEND_API_KEY : API key lấy từ https://resend.com (miễn phí 100 email/ngày)
//   EMAIL_FROM      : địa chỉ gửi đi, vd "SNG EDU <hotro@tenmiendomoi.site>"
//                      (domain phải verify trên Resend)
//   SITE_URL        : domain chính của site, vd "https://sngedu.site" — dùng để
//                      chèn link "Vào SNG EDU ngay" trong email. Đổi domain
//                      thì CHỈ CẦN sửa secret này, không cần sửa code.
// ============================================================

export const RESEND_API_KEY = Deno.env.get("RESEND_API_KEY");
export const EMAIL_FROM = Deno.env.get("EMAIL_FROM") ?? "SNG EDU <onboarding@resend.dev>";
export const SITE_URL = Deno.env.get("SITE_URL") ?? "https://sngedu.site";
...
export function welcomeEmailHtml(name: string) {
  ...
  const body = `
    ...
    <div style="text-align:center;margin:0 0 8px;">
      <a href="${SITE_URL}" style="display:inline-block;padding:12px 28px;background:#5b5cf0;color:#fff;text-decoration:none;border-radius:10px;font-weight:700;font-size:14px;">Vào SNG EDU ngay</a>
    </div>
