...
//   - Trang > free_pages, tài liệu Trả phí hoặc VIP -> phải đăng nhập VÀ đang có Pro,
//     nếu không sẽ trả 403 (VIP không có đường mua lẻ, chỉ Pro mới mở khoá xem tiếp).
...
// Giống hệt logic isProActive() trong assets/js/usage-limits.js —
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const payload = settingsRow.payload || { free: [], paid: [], vip: [] };
    const paidList: any[] = Array.isArray(payload.paid) ? payload.paid : [];
    const vipList: any[] = Array.isArray(payload.vip) ? payload.vip : [];
    const freeList: any[] = Array.isArray(payload.free) ? payload.free : [];

    let item = paidList.find((it) => String(it.id) === docId);
    let isGatedDoc = !!item; // trả phí HOẶC vip -> đều phải qua kiểm tra Pro bên dưới
    if (!item) item = vipList.find((it) => String(it.id) === docId);
    if (item) isGatedDoc = true;
    ...
    // Áp dụng như nhau cho tài liệu Trả phí và tài liệu VIP (VIP vốn không có đường mua lẻ).
    let hasFullAccess = !isGatedDoc; // tài liệu miễn phí -> full luôn, không giới hạn
    if (isGatedDoc && user && isProActive(user)) {
      hasFullAccess = true;
    }
