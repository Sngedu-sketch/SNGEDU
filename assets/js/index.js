const toast = document.getElementById('toast');
    ...
    const DEFAULT_DOC_CONTENT = { free: [], paid: [], vip: [] };
    ...
    function docItemCardHtml(item, kind){
        // kind: 'free' | 'paid' | 'vip'. Hỗ trợ giá trị cũ (boolean isPaid) để không phá các chỗ gọi khác.
        if (kind === true) kind = 'paid';
        if (kind === false) kind = 'free';
        ...
        const topHtml = item.image
            ...
            : '';
        const iconHtml = `<div class="doc-icon"><i class="fa-solid ${kind === 'vip' ? 'fa-crown' : (kind === 'paid' ? 'fa-lock' : 'fa-file-lines')}"></i></div>`;
        ...
        // Tài liệu VIP: không bán lẻ, không có link tải file gốc — chỉ xem theo trang (đọc trực tuyến).
        // Bấm vào thẻ -> mở trang chi tiết; trang đó dẫn tiếp qua doc-preview-viewer.html để xem,
        // Pro mới xem hết trang (chưa có Pro vẫn xem được vài trang đầu nếu admin cho phép).
        if (kind === 'vip'){
            const idAttr = escapeHtmlHome(String(item.id || ''));
            const disabled = !(item.preview_mode === 'pages' && item.pages_ready);
            return `
                <a class="doc-card is-vip${disabled ? ' is-disabled' : ''}" data-cat="vip" href="${disabled ? '#' : `chi-tiet.html?type=doc&id=${idAttr}`}" ${disabled ? 'onclick="return false;"' : ''}>
                    <div class="${topClass}">
                        ${topHtml || iconHtml}
                        <span class="doc-tag vip"><i class="fa-solid fa-crown"></i> VIP</span>
                    </div>
                    <div class="doc-title">${escapeHtmlHome(item.title || '')}</div>
                    ${metaHtml}
                    ${descHtml}
                    <div class="doc-cta">${disabled
                        ? '<i class="fa-solid fa-hourglass-half"></i> Sắp ra mắt'
                        : '<i class="fa-solid fa-eye"></i> Xem tài liệu'}</div>
                </a>`;
        }
    ...
    async function renderDocContent(){
        ...
        const vip = (Array.isArray(payload.vip) ? payload.vip : []).filter(it => !it.hidden);
        DOC_PAID_ITEMS_MAP = {};

        const allGrid = document.getElementById('docAllGrid');
        const allCount = document.getElementById('docAllCount');
        const total = free.length + paid.length + vip.length;
        if (allCount) allCount.textContent = total + ' tài liệu';

        const cardsHtml = free.map(it => docItemCardHtml(it, 'free')).join('')
            + paid.map(it => docItemCardHtml(it, 'paid')).join('')
            + vip.map(it => docItemCardHtml(it, 'vip')).join('');
    ...
    async function confirmDocPurchase(useBalance){
        ...
        thanhToanTaiLieu(docConfirmSelectedId); // hàm có sẵn trong assets/js/sepay-checkout.js
    }
    ...
    async function xuLyThanhToanBangSoDu({ orderPayload, balanceBtnId, onSuccess }){
        ...
        const result = await thanhToanBangSoDu(orderPayload); // hàm có sẵn trong assets/js/sepay-checkout.js
        if (!result.ok){
    ...
    function submitNapTien(){
        ...
        const okBtn = document.getElementById('napTienOkBtn');
        if (okBtn){ okBtn.disabled = true; okBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Đang chuyển tới cổng thanh toán...'; }
        napTienVaoVi(amount); // hàm có sẵn trong assets/js/sepay-checkout.js — chuyển sang cổng SePay
    }
    ...
    function purchaseProductFromModal(id){
        ...
        thanhToanSanPham(id); // hàm có sẵn trong assets/js/sepay-checkout.js
    }
    ...
    async function renderFeaturedExtras(){
        ...
        if (docIds.length){
            ...
            const freeHtml = docIds.filter(id => docsMap[id] && !docsMap[id].isPaid).map(id => docItemCardHtml(docsMap[id].item, 'free')).join('');
            const paidHtml = docIds.filter(id => docsMap[id] && docsMap[id].isPaid).map(id => docItemCardHtml(docsMap[id].item, 'paid')).join('');
    ...
    async function confirmProUpgrade(useBalance){
        ...
        const okBtn = document.getElementById('proConfirmOkBtn');
        if (okBtn){ okBtn.disabled = true; okBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Đang chuyển tới cổng thanh toán...'; }
        thanhToanGoiThanhVien(proUpgradeSelectedId); // hàm có sẵn trong assets/js/sepay-checkout.js
    }
