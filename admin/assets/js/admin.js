const SUPABASE_URL = window.SNG_SUPABASE_URL;
const SUPABASE_ANON_KEY = window.SNG_SUPABASE_ANON_KEY;
...
// gắn sẵn assets/js/maintenance-check.js, tự đọc key này và chuyển sang bao-tri.html khi enabled=true.
...
const CONTENT_MANAGER_GROUPS = {
    doc:     { title:'Tài liệu',  icon:'📁',  hint:'Quản lý danh sách tài liệu (miễn phí, trả phí & VIP) hiển thị ở tab "Tài liệu" trên trang chủ. Tải file PDF/Word lên trực tiếp, không cần link ngoài. Chọn loại "Trả phí" và bắt buộc nhập "Giá bán" — khách phải thanh toán qua SePay thì nút mới đổi thành "Tải xuống", trước đó file không lộ link công khai. Chọn loại "VIP" cho tài liệu không bán lẻ — chỉ khách đang có Pro/VIP mới xem/tải được, khách chưa có Pro sẽ thấy nút "Nạp VIP để xem" dẫn tới nâng cấp Pro. Bấm biểu tượng mắt 👁️ ở mỗi dòng để ẩn/hiện tạm thời mà không cần xoá — khách đã mua vẫn tải được bình thường khi bị ẩn.', subKeys:['doc'] },
            ...
            <div class="subj-mgr-col">
                <div id="cmForm_${subKey}" class="mini-form">
                    ...
                    <div class="doc-type-toggle" id="cmDocType_${subKey}">
                        ...
                        <div class="doc-type-opt vip" data-type="vip" onclick="pickDocType('${subKey}','vip')"><i class="fa-solid fa-crown"></i> VIP</div>
                    </div>
                    <div id="cmPriceRow_${subKey}">
                    ...
                    <div class="hint" id="cmVipHint_${subKey}" style="margin:10px 0 0;display:none;">Tài liệu VIP không bán lẻ — chỉ khách đang có Pro/VIP mới xem/tải được. Khách chưa có Pro sẽ thấy nút "Nạp VIP để xem" dẫn tới nâng cấp Pro.</div>` : ''}
                    ...
                    <div class="hint" id="cmVipFileHint_${subKey}" style="display:none;margin-top:4px;">Tài liệu VIP không cần tải file ở đây — nội dung được nạp riêng qua công cụ <b>"Xem trước theo trang"</b> (menu Tài liệu &gt; Xem trước theo trang) sau khi lưu tài liệu này, để không lộ file gốc.</div>
...
async function loadContentList(subKey){
    ...
    if (cfg.meta.docTypeField){
        // Tài liệu: gộp 3 mảng free/paid/vip (định dạng lưu trữ cũ, trang chủ vẫn đọc riêng 3 mảng này)
        ...
        const vipArr = Array.isArray(payload.vip) ? payload.vip : [];
        arr = freeArr.map(x => Object.assign({}, x, { type: 'free', price: 0 }))
            .concat(paidArr.map(x => Object.assign({}, x, { type: 'paid' })))
            .concat(vipArr.map(x => Object.assign({}, x, { type: 'vip', price: 0 })));
    } else {
...
function contentItemRowHtml(subKey, item){
    ...
    if (cfg.docTypeField){
        ...
        const typeLabel = isVip ? 'VIP' : (isPaid ? 'Trả phí' : 'Miễn phí');
        const typeCls = isVip ? '' : (isPaid ? 'maintenance' : 'ready');
        typeBadgeHtml = `<span class="status-badge ${typeCls}" style="margin-right:6px;${isVip ? 'background:rgba(139,92,246,.14);color:#8b5cf6;' : ''}">${typeLabel}</span>`;
...
function pickDocType(subKey, type){
    ...
    const priceRowEl = document.getElementById('cmPriceRow_' + subKey);
    const vipHintEl = document.getElementById('cmVipHint_' + subKey);
    // Tài liệu VIP không bán lẻ -> ẩn hẳn ô giá, không hiện gợi ý nhập giá.
    if (priceRowEl) priceRowEl.style.display = type === 'vip' ? 'none' : '';
    if (vipHintEl) vipHintEl.style.display = type === 'vip' ? '' : 'none';
    ...
    // Tài liệu VIP: nội dung thật nạp qua công cụ "Xem trước theo trang" riêng (tách ảnh từng trang,
    // không lộ file gốc) -> ẩn hẳn khối "Tệp & ảnh bìa" ở đây, khỏi cần chọn file/link.
    const fileSectionEl = document.getElementById('cmFileSection_' + subKey);
    const vipFileHintEl = document.getElementById('cmVipFileHint_' + subKey);
    if (fileSectionEl) fileSectionEl.style.display = type === 'vip' ? 'none' : '';
    if (vipFileHintEl) vipFileHintEl.style.display = type === 'vip' ? '' : 'none';
...
function updateContentPreview(subKey){
    ...
    if (cfg.meta.docTypeField){
        ...
        const isVip = docType === 'vip';

        const tagEl = document.getElementById('cmPrevTag_' + subKey);
        if (tagEl){
            tagEl.className = 'dcp-tag' + (isVip ? ' vip' : (isPaid ? ' paid' : ''));
            tagEl.innerHTML = isVip
                ? '<i class="fa-solid fa-crown"></i> VIP'
                : (isPaid ? '<i class="fa-solid fa-lock"></i> Trả phí' : '<i class="fa-solid fa-gift"></i> Miễn phí');
        }
        ...
        if (ctaEl){
            ...
            ctaEl.className = 'dcp-cta' + (isVip ? ' vip' : (isPaid ? ' paid' : ''));
            ctaEl.innerHTML = isVip
                ? '<i class="fa-solid fa-crown"></i> Nạp VIP để xem'
                : (isPaid
        ...
        if (thumbEl && iconEl2){
            if (imgUrl){
                ...
                iconEl2.innerHTML = '<i class="fa-solid ' + (isVip ? 'fa-crown' : (isPaid ? 'fa-lock' : 'fa-file-lines')) + '"></i>';
            }
        }
...
function editContentItem(subKey, id){
    ...
    if (cfg.meta.docTypeField) pickDocType(subKey, item.type === 'paid' ? 'paid' : (item.type === 'vip' ? 'vip' : 'free'));
...
async function saveContentList(subKey){
    ...
    if (cfg.meta.docTypeField){
        // Tách lại danh sách gộp thành 3 mảng free/paid/vip để trang chủ (index.html) đọc như cũ.
        payload.free = cfg.items.filter(it => it.type !== 'paid' && it.type !== 'vip').map(it => {
            const { type, ...rest } = it;
        ...
        payload.vip = cfg.items.filter(it => it.type === 'vip').map(it => {
            const { type, ...rest } = it;
            return Object.assign({}, rest, { price: 0 });
        });
    } else {
